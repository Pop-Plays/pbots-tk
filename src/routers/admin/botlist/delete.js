...
app.get("/admin/delete/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(botdata.ownerID).then(sahip => {
        client.channels.cache.get(channels.botlog).send(new Discord.MessageEmbed().setColor("RED").setDescription(`[**<@${botdata.botID}>** got deleted from Pbots.tk. `).addField("Bot owner", `<@${botdata.ownerID}>`))
        client.users.cache.get(botdata.ownerID).send(`Your bot named **${botdata.username}** has been deleted.`)
    })
