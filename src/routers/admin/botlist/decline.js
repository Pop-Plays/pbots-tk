...
app.post("/admin/decline/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(botdata.ownerID).then(sahip => {
        client.channels.cache.get(channels.botlog).send(new Discord.MessageEmbed().setColor("RED").setDescription(`[**<@${botdata.botID}>** got declined from Pbots.tk. `).addField("Reviewer", `<@${req.user.id}>`).addField("Reason", `\`${rBody['reason']}\``).addField("Bot owner", `<@${botdata.ownerID}>`))
