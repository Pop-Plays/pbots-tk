...
console.log("[vcodes.xyz]: Botlist/Add Bot router loaded.");
    ...
    app.post("/addbot", global.checkAuth, async (req,res) => {
      ...
      var longDesc = converter.makeHtml(rBody['longDesc']);
      ...
      client.channels.cache.get(channels.botlog).send(new Discord.MessageEmbed().setColor("BLUE").setDescription(`**<@${a.id}>** was added to Pbots.tk. `).setThumbnail(`https://cdn.discordapp.com/avatars/${a.id}/${a.avatar}`).addField("Bot owner", `<@${req.user.id}>`))
